    ...
    <section id="debt-composition" className="border-b border-[#1a1a1a] bg-[#050505]">
      <div className="max-w-[1440px] mx-auto px-4 py-8" ref={chartRef}>
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-2"><div><h2 className="text-xl font-bold text-[#e0e0e0] tracking-tight">{t("section5.title")}</h2><p className="text-[11px] font-mono text-[#555] mt-1">{t("section5.subtitle")}</p></div></div>
          <div className="flex items-center gap-2"><ExportButton chartRef={chartRef} filename="debt-composition" jsonData={jsonData} /><button onClick={() => onEmbedClick("debt-composition")} className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-mono text-[#555] hover:text-[#00FFFF] transition-colors border border-[#222] hover:border-[#00FFFF]/40"><Share2 size={12} /></button></div>
        </div>
        ...
        <div className="mt-6 border border-[#1a1a1a]">
          ...
          <div className="grid grid-cols-12 gap-0 bg-[#111] border-b border-[#1a1a1a] text-[9px] font-mono uppercase tracking-widest text-[#555] py-2 px-3">
            <div className="col-span-3">{t("spreads.country")}</div><div className="col-span-2 text-right">{t("section5.official")}</div><div className="col-span-2 text-right">{t("section5.estimated")}</div><div className="col-span-2 text-right">{t("section5.vsEst")}</div><div className="col-span-3">{t("sourceOverlay.title")}</div>
          </div>
          {chartData.map((c, i) => (
            <div key={c.countryEn} className={`grid grid-cols-12 gap-0 py-2 px-3 text-[11px] font-mono ${i < chartData.length - 1 ? "border-b border-[#111]" : ""} hover:bg-[#0e0e0e] transition-colors`}>
              ...
              <div className="col-span-3 text-[8px] text-[#444]">{c.debtToGDPSource} &middot; {c.debtToGDPSnapshot}</div>
            </div>
          ))}
        </div>
