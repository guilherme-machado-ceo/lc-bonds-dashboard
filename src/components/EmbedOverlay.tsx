...
export default function EmbedOverlay({ sectionId, onClose }: EmbedOverlayProps) {
  ...
  const iframeCode = `<iframe\n  src="${typeof window !== "undefined" ? window.location.origin : "https://hubstry.dev"}/embed/${sectionId}"\n  width="100%"\n  height="500"\n  frameborder="0"\n  style="background:#0a0a0a;"\n  title="LC Bonds Dashboard"\n></iframe>`;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#0a0a0a] border border-[#222] w-full max-w-xl mx-4 p-6 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-[#111] border border-[#222] p-3 mb-4">
          <pre className="text-[11px] font-mono text-[#888] overflow-x-auto whitespace-pre-wrap break-all">{iframeCode}</pre>
        </div>
        <div className="flex items-center justify-between">
          <button onClick={handleCopy} className="flex items-center gap-2 px-4 py-1.5 text-[11px] font-mono border border-[#222] text-[#aaa] hover:text-[#00FFFF] hover:border-[#00FFFF]/40 transition-colors">
            {copied ? <Check size={12} className="text-[#00FF88]" /> : <Copy size={12} />}
            {copied ? t("embedOverlay.copied") : t("embedOverlay.copy")}
          </button>
