  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#0a0a0a] border border-[#222] w-full max-w-lg mx-4 p-6 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-4">
          <span className="text-[10px] font-mono uppercase tracking-widest text-[#00FFFF]">{t("sourceOverlay.title")}</span>
        </div>
        <h3 className="text-lg font-semibold text-[#e0e0e0] mb-2">{source.name}</h3>
        <div className="mb-4">
          <a href={source.url} target="_blank" rel="noopener noreferrer" className="text-[11px] font-mono text-[#00FFFF] hover:underline flex items-center gap-1"><ExternalLink size={10} />{source.url}</a>
        </div>
