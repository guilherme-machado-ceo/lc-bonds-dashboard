...
export default function TCXChart({ onSourceClick, onEmbedClick }: Props) {
  ...
  const jsonData = { tcx: tcxHedgingData, summary: { currencies: kpis.tcxCurrencies, portfolioLatest: 8.1, snapshot: "2025e" } };
    ...
    <section id="tcx" className="border-b border-[#1a1a1a] bg-[#050505]">
      <div className="max-w-[1440px] mx-auto px-4 py-8" ref={chartRef}>
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-2"><div><div className="flex items-center gap-2 mb-1"><EstBadge /></div><h2 className="text-xl font-bold text-[#e0e0e0] tracking-tight">{t("section4.title")}</h2><p className="text-[11px] font-mono text-[#555] mt-1">{t("section4.subtitle")} &middot; {kpis.tcxCurrencies} {t("hero.kpiCurrencies")} &middot; 2025e</p></div></div>
          <div className="flex items-center gap-2"><ExportButton chartRef={chartRef} filename="tcx-hedging" jsonData={jsonData} /><button onClick={() => onEmbedClick("tcx")} className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-mono text-[#555] hover:text-[#00FFFF] transition-colors border border-[#222] hover:border-[#00FFFF]/40"><Share2 size={12} /></button></div>
        </div>
        <div className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={tcxHedgingData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <Line yAxisId="left" type="monotone" dataKey="annualHedged" name={t("section4.seriesHedged")} stroke="#00FFFF" strokeWidth={1.5} dot={false} activeDot={{ r: 4, fill: "#00FFFF" }} />
              <Line yAxisId="left" type="monotone" dataKey="portfolioOutstanding" name={t("section4.seriesOutstanding")} stroke="#e0e0e0" strokeWidth={1.5} dot={false} activeDot={{ r: 4, fill: "#e0e0e0" }} />
              <Line yAxisId="right" type="monotone" dataKey="currencies" name={t("section4.seriesCurrencies")} stroke="#FF8C00" strokeWidth={1} strokeDasharray="4 4" dot={false} activeDot={{ r: 3, fill: "#FF8C00" }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
